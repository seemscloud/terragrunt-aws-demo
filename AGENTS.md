...
This repository is a Terragrunt/OpenTofu AWS infrastructure project. Reusable Terraform code lives in `modules/`, with one focused AWS resource type per module, while deployable state leaves live under `environments/theanotherwise/<region>/...`. The workload regional footprint is `eu-west-1`, `eu-west-2`, and `eu-west-3`; `eu-central-1` is used only as the Transit Gateway hub for those three regions. Resource type belongs in the structure or module, not in the logical instance name: use paths such as `compute/eks/primary` and `network/vpc/primary`, and keep the logical instance name as `primary`.

The primary regional layout uses `network/vpc/primary/*` for VPC, subnet, NAT, route table, route, route table association, and network ACL states. Regional transit gateways live under `network/transit-gateway/*`; each workload VPC attaches once via `vpc-attachment/primary`, spoke regions accept one peering from `eu-central-1`, and the hub creates one requester peering per spoke under `eu-central-1/network/transit-gateway/peerings/<spoke-region>/requester`. TGW route table routes live under `network/transit-gateway/routes`; spoke TGWs route every remote workload CIDR to the central hub peering, and the central hub routes each workload CIDR back to the owning spoke peering.

Private DNS for EKS workloads is represented under regional `dns/eks/primary/*`. Each private hosted zone has its own `private-zone` leaf, workload records live under `records/<record-name>`, and non-home VPC links live under `vpc-associations/<region>-<vpc-name>` so adding another VPC or region does not require rebuilding the zone leaf. Regional EKS public API access is intentionally open to `0.0.0.0/0`, with private endpoint access enabled for VPC-local access. Regional management hosts allow SSH from the internet, but mgmt subnet network ACLs deny SSH sourced from local EKS node subnets. Inter-region VPC routes and TGW routes should stay scoped to the specific load balancer and EKS private subnets required for cluster-to-cluster service traffic, not whole VPC CIDR blocks or peer management subnets. Regional `compute/eks/primary/addons/vpc-cni` states manage the EKS `vpc-cni` add-on and exclude remote load balancer CIDRs from SNAT so cross-region service calls preserve pod source IPs; NLB-backed test services that need source IP visibility use client IP preservation and `externalTrafficPolicy = Local`. While pods still use the default AWS VPC CNI subnet addressing, remote EKS private subnet routes and NACL allows are required as the response path for preserved pod source IPs.

...
dependency "node_role" {
  ...
  mock_outputs = {
    role_arn  = "arn:aws:iam::674403012652:role/primary-eu-west-1-node"
    role_name = "primary-eu-west-1-node"
  }
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan", "destroy"]
  mock_outputs_merge_strategy_with_state  = "deep_map_only"
}
...
dependency "node_launch_template" {
  ...
  mock_outputs = {
    launch_template_ids = {
      "primary-eu-west-1-node" = "lt-0123456789abcdef0"
    }
    launch_template_latest_versions = {
      "primary-eu-west-1-node" = "1"
    }
  }
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan", "destroy"]
  mock_outputs_merge_strategy_with_state  = "deep_map_only"
...
inputs = {
  eks_node_groups = {
    "primary-eu-west-1a" = {
      ...
      launch_template = {
        id      = dependency.node_launch_template.outputs.launch_template_ids["primary-eu-west-1-node"]
        version = dependency.node_launch_template.outputs.launch_template_latest_versions["primary-eu-west-1-node"]
      }
      tags = {
